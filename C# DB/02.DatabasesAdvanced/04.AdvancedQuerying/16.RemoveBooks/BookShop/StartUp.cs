namespace BookShop
{
    ...
    public class StartUp
    {
        ...
        {
            var allBookAndPrice = context.Categories
                .Select(x => new
                {
                    categoryName = x.Name,
                    totalPrice = x.CategoryBooks.Select(x => x.Book.Price * x.Book.Copies).Sum()
                })
            ...
            foreach (var book in allBookAndPrice)
            {
                sb.AppendLine($"{book.categoryName} ${book.totalPrice:f2}");
            }
        ...
        public static string GetMostRecentBooks(BookShopContext context)
        {
            var categories = context.Categories
                 ...
                 {
                     categoryName = x.Name,
                     books = x.CategoryBooks.OrderByDescending(x => x.Book.ReleaseDate)
                                                         .Select(b => new
                                                         {
                                                             bookTittle = b.Book.Title,
                                                             bookRelease = b.Book.ReleaseDate.Value
                                                         })
                                                         .Take(3)
            ...
            foreach (var category in categories)
            {
                ...
                {
                    sb.AppendLine($"{book.bookTittle} ({book.bookRelease.Year})");
                }
            }
            return sb.ToString().Trim();
        }
        ...
        {
            int removeBooksCount = context.Books.Where(x => x.Copies < 4200).Count();
            var books = context.Books.Where(x => x.Copies < 4200).ToList();

            context.Books.RemoveRange(books);
            context.SaveChanges();

            return removeBooksCount;
        }
    }
}
